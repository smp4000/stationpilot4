...
<head>
  ...
  <title>{{ $document->template->name ?? 'Dokument' }} unterschreiben</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .sig-controls { display: flex; gap: 10px; margin-top: 10px; flex-wrap: wrap; align-items: center; }
    ...
    .success-banner { background: #d1fae5; border: 1px solid #6ee7b7; border-radius: 8px; padding: 20px 24px; text-align: center; margin-bottom: 20px; }
    .success-banner h2 { color: #065f46; font-size: 16px; }
    .success-banner p { color: #047857; margin-top: 6px; font-size: 13px; }
    .already-signed { background: #fef9c3; border: 1px solid #fde047; border-radius: 8px; padding: 16px 20px; text-align: center; margin-top: 20px; }
...
<div class="top-bar">
  <div>
    <div class="title">StationPilot · Dokument unterschreiben</div>
...
<div class="container">
  ...
  <div class="success-banner">
    <h2>✅ Vielen Dank — Dokument erfolgreich unterschrieben!</h2>
    <p>Ihre Unterschrift wurde gespeichert. Sie können dieses Fenster schließen.</p>
  </div>
  @endif

  <div class="card">
    <div class="card-header">
      <h2>{{ $document->template->name ?? 'Dokument' }}</h2>
      <p>
        Erstellt am {{ $document->generated_at->format('d.m.Y') }}
        @if($document->signed_at)
          &nbsp;·&nbsp; <span style="color:#059669;">✓ Unterschrieben am {{ $document->signed_at->format('d.m.Y H:i') }}</span>
        @endif
      </p>
    </div>
    <div class="card-body">
      ...
      {{-- PDF Vorschau --}}
      <iframe src="{{ route('document.sign.pdf', $document->sign_token) }}"></iframe>

      {{-- Unterschrift --}}
      @if($document->signed_at)
        <div class="already-signed">
          <p>✅ Sie haben dieses Dokument bereits am <strong>{{ $document->signed_at->format('d.m.Y \u\m H:i \U\h\r') }}</strong> unterschrieben.</p>
        </div>
      @else
        <div class="sig-section">
          ...
          <div class="sig-controls">
            ...
            <form method="POST" action="{{ route('document.sign.submit', $document->sign_token) }}" id="sigForm" style="display:flex;gap:10px;align-items:center;">
              @csrf
          ...
          <p class="hint">Mit dem Absenden bestätigen Sie, dass Sie das Dokument gelesen haben und mit dem Inhalt einverstanden sind.</p>
        </div>
      @endif
