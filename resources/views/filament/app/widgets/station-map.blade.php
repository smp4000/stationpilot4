        ...
        @else
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    const map = L.map('station-overview-map');
                    ...
                    stations.forEach(s => {
                        ...
                        bounds.push([lat, lng]);
                        const addr = [s.street + ' ' + (s.house_number || ''), s.zip + ' ' + s.city].join(', ');
                        L.marker([lat, lng], {icon}).bindPopup(
                            `<strong style="color:#003B95">${s.name}</strong>${s.brand ? `<br><small>${s.brand}</small>` : ''}<br><small>${addr}</small>`
                        ).addTo(map);
                    });
